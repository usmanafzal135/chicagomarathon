...
?>
    <div 
        class="js-hidden announcement-bar delay-500 alpine-js flex flex-col md:flex-row justify-center items-center relative block bg-primary pr-3 pl-2 py-half"
        x-data="announcementData()"
        x-show="open"
    >
        <?php if ($announcement) { ?>
            <div class="flex items-center">
                <i class="fill-white mr-[4px]">
                ...
                <p class="text-white text-sm text-center mb-0"><?php echo $announcement; ?></p>
            </div>
        <?php } ?>
        ...
        <button 
            ...
            class="absolute <?php  if ($link) { ?> top-half <?php } else {  ?> top-auto <?php } ?>right-1 md:top-auto p-[4px] mb-0 !bg-transparent !border-none !shadow-none transition duration-500 ease-in-out hover:rotate-90"
        >
            <i class="fill-white">
